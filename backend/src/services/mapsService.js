...
export const getDistanceMatrix = async (originCoords, destCoords) => {
    ...
    if (!env.googleMapsKey || env.googleMapsKey === 'your_google_maps_api_key') {
        logger.warn('Using open-source OSRM API for routing due to missing Google Maps key');
        try {
...
export const geocodeAddress = async (address) => {
    if (!env.googleMapsKey || env.googleMapsKey === 'your_google_maps_api_key') {
        logger.warn('Using open-source Nominatim API for geocoding due to missing Google Maps key');
        try {
