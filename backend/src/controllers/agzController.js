...
export const getSession = async (req, res, next) => {
  try {
    ...
    let session = await AGZSession.findOne({ rideRequest: requestId }).populate(
      {
        path: "rideRequest",
        populate: [{ path: "ride", populate: "driver" }, { path: "passenger" }],
      },
    );
    ...
    const isRider =
      sessionObj.rideRequest.passenger._id.toString() ===
      req.user._id.toString();
    const isDriver =
      sessionObj.rideRequest.ride.driver._id.toString() ===
      req.user._id.toString();

    // Enforce Partial Visibility
    if (sessionObj.currentStage !== "UNLOCKED") {
      if (isRider && sessionObj.rideRequest.ride.driver) {
        sessionObj.rideRequest.ride.driver.phoneNumber =
          "HIDDEN (Pay to Unlock)";
      }
...
export const confirmInterest = async (req, res, next) => {
  try {
    const { requestId } = req.params;
    const session = await AGZSession.findOne({
      rideRequest: requestId,
    }).populate("rideRequest");
    if (!session) return res.status(404).json({ message: "Session not found" });

    const isPassenger =
      session.rideRequest.passenger.toString() === req.user._id.toString();
    const isDriver =
      session.rideRequest.ride &&
      session.rideRequest.ride.driver.toString() === req.user._id.toString();

    if (isPassenger) session.isRiderConfirmed = true;
    if (isDriver) session.isDriverConfirmed = true;

    if (
      session.isRiderConfirmed &&
      session.isDriverConfirmed &&
      ...
      await ChatMessage.create({
        ...
        sender: req.user._id,
        content:
          "⚡ BOTH PEERS HAVE CONFIRMED INTEREST! The commitment gate is now open. Please proceed to settle the platform fee to unlock direct coordination.",
