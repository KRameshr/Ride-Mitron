...
export const postRide = async (req, res, next) => {
  try {
    ...
    const user = await User.findById(req.user._id)
      .select("vehicleDetails totalRidesGiven")
      .lean();
    if (!user || !user.vehicleDetails || !user.vehicleDetails.hasVehicle) {
      return res
        .status(403)
        .json({ message: "Must add vehicle details to post a ride" });
    }

    let { origin, destination, startTime, totalSeats, manualDistance } =
      req.body;
    ...
    if (
      ...
      destination.coordinates[0] == null
    ) {
    ...
    if (distanceKm > 0) {
      ...
      driverFee = Math.max(
        config.driverMinFee,
        fuelCostPerUser * ((config.driverPercentage || 5) / 100),
      );
      riderFee = Math.max(
        config.riderMinFee,
        fuelCostPerUser * ((config.riderPercentage || 10) / 100),
      );
    ...
    logger.info(
      `Ride Posted: ID ${newRide._id}, Origin: ${origin.name} ${origin.coordinates}, Dest: ${destination.name}`,
    );

    // User update mechanics
    await User.findByIdAndUpdate(req.user._id, {
      $inc: { totalRidesGiven: 1 },
    });
...
export const searchRides = async (req, res, next) => {
  try {
    ...
    logger.info(
      `Processing Search: Origin: ${originName || "any"}, Dest: ${destName || "any"}, Date: ${date}, Seats: ${seats}. Query: ${JSON.stringify(query)}`,
    );

    const rides = await Ride.find(query)
      .select(
        "origin destination startTime availableSeats vehicleType costPerSeat driver",
      )
    ...
    res
      .status(500)
      .json({ message: "Internal server error during search optimization." });
  }
};
...
export const updateRideStatus = async (req, res, next) => {
  try {
    ...
    if (ride.driver.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Only driver can update ride status" });
    }
    ...
    if (status === "COMPLETED") {
      ...
      for (const request of requests) {
        await User.findByIdAndUpdate(request.passenger, {
          $inc: { totalRidesTaken: 1 },
        });
      }
    }
...
export const getRideDetails = async (req, res, next) => {
  try {
    const ride = await Ride.findById(req.params.id)
      .populate(
        "driver",
        "name phoneNumber rating totalRidesGiven vehicleDetails",
      )
      .select("-__v")
      .lean();
    ...
    const isSelf =
      req.user && ride.driver._id.toString() === req.user._id.toString();
    ...
    if (!isSelf) {
      ...
      if (req.user) {
        ...
        const paidRequest = await RideRequest.findOne({
          ...
          $or: [{ status: "ACCEPTED" }, { riderPaid: true }],
        }).lean();
      ...
      const acceptedPassengers = await RideRequest.find({
        ...
        .populate("passenger", "name phoneNumber rating")
        .lean();
...
export const cancelRide = async (req, res, next) => {
  try {
    ...
    if (ride.driver.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Only driver can cancel the ride" });
    }
